...
\centering
\includegraphics[width=0.7\textwidth]{mesure_physique/ina219.jpg}
\caption{Arduino INA219 High Side DC Current Sensor}
\label{shuntphoto}
\centering
\includegraphics[width=0.7\textwidth]{mesure_physique/arduinoUno.jpg}
...
Comme évoqué dans le paragraphe \ref{presentationmodulephy}, il a fallu adapter la sonde physique dans le but de mesurer la consommation d'énergie sur mobile. L'idée est de placer le shunt entre le téléphone et la batterie et de mesurer le courant sortant de cette dernière vers l'appareil.  
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{mesure_physique/connecteursbatt.png}
\caption{Connecteurs batterie}
\label{connecteurs}
\end{figure}
...
\code{WebDriverAgent}\footnote{\url{https://github.com/facebook/WebDriverAgent}} est un outil développé par \textsc{Facebook}. Il s'agit d'un serveur WebDriver pour iOS permettant de contrôler des appareils à distance. Il permet notamment de lancer des applications (y compris celles de l'App Store) et effectuer des actions sur l'écran (scroller, taper au clavier...), ce qui répond à notre besoin.
...
Pour les étapes de test je me suis basé sur les tests existants sur Android. Les scripts sont présentés dans l'annexe \ref{sec:iosscripts}. 
...
\item Lancement du navigateur et \textbf{étape de référence} : le mobile est inactif sur l'accueil du navigateur pendant 20 secondes\footnote{Les scripts présentent des durées de 5 secondes pour  les rendre moins longs à tester.}
...
Les URLs du serveur \code{WebDriverAgent} et du site web mesuré sont passées en argument de ce script.

\subsubsection{Benchmark d'application}
Le benchmark d'application comporte 6 étapes\footnote{L'étape \textbf{idle foreground} n'a pas été écrite dans le script mais elle est présente dans l'implémentation finale.} : 
\begin{itemize}[noitemsep]
...
Ici on passe en argument le chemin vers le fichier \code{.app} et l'identifiant de l'application (par exemple, pour le navigateur d'Apple : \code{com.mobilesafari}) en plus de l'URL du serveur \code{WebDriverAgent}.


\bigskip Je me suis servi de ces scripts de benchmark comme base de travail pour l'implémentation de la fonctionnalité iOS dans \code{go-testapi} et \code{test runner}. 
...
Ensuite, dans un autre fichier, j'ai créé les structures \code{IosDeviceConfig} (contenant les informations relatives à un appareil) et \code{IosDevice} (contenant une instance de \code{IosDeviceConfig}) qui implémente l'interface \code{Device} (déclaration fig. \ref{iosdevice}). Pour implémenter les méthodes de l'interface, j'ai adapté mes tests de \code{WebDriverAgent} en effectuant les différentes nécessaires en \code{go}, ce qui rend les interactions plus commodes. En effet, un appel de méthode est beaucoup plus simple et explicite qu'une ligne de commande \code{cURL}. 
...
\begin{itemize}[noitemsep]
\item Lancement de \code{WebDriverAgent} sur l'appareil
\item Démarrage de la mesure physique
\item Déroulement des tests sur une ou plusieurs itérations\footnote{On note que dans le cas d'un benchmark d'application, l'installation et la désinstallation ne sont pas effectuées à chaque itération mais seulement avant et après l'exécution de tous les tests.} : voir la section \ref{scriptsbenchmark}
